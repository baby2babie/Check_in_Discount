...
function startGachaponOpen(token) {
  ...
  // first (biggest) bounce impact lands at 50% of the .95s bounce, i.e. ~825ms from crank-press
  setTimeout(() => {
  ...
  // flap opens once the capsule has fully settled (350ms delay + 950ms bounce)
  setTimeout(() => {
...
function gpShowResult(result) {
  ...
  const rays = document.getElementById('gpRays');
  if (rays) { rays.classList.remove('go'); void rays.offsetWidth; rays.classList.add('go'); }
...
function closePaidOverlay() {
  ...
  const rays = document.getElementById('gpRays');
  if (rays) rays.classList.remove('go');
